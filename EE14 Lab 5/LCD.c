...
void LCD_Configure(void){
	...
	LCD->CR &= ~(LCD_CR_BIAS); //clear bits of the bias
	LCD->CR |= LCD_CR_BIAS_1; //set bit 1 to 1
	
	// Configure DUTY[2:0] of LCD_CR and set duty to 1/4 (011)
	LCD->CR &= ~(LCD_CR_DUTY); //clear bits of duty
	LCD->CR |= LCD_CR_DUTY_1; //set bit 0 to 1
	LCD->CR |= LCD_CR_DUTY_2; //set bit 1 to 1
	
	// Configure CC[2:0] bits of LCD_FCR and set contrast value to maximum (111)
	LCD->FCR &= ~(LCD_FCR_CC); //clear bits of contrast
	LCD->FCR |= LCD_FCR_CC_0; //set bit 0 to 1
	LCD->FCR |= LCD_FCR_CC_1; //set bit 1 to 1
	LCD->FCR |= LCD_FCR_CC_2; //set bit 2 to 1
	
	// Configure PON[2:0] bits of LCD_FCR and set pulse period to maximum (111)
	LCD->FCR &= ~(LCD_FCR_PON); //clear bits of pulse
	LCD->FCR |= LCD_FCR_PON_0; //set bit 0 to 1
	LCD->FCR |= LCD_FCR_PON_1; //set bit 1 to 1
	LCD->FCR |= LCD_FCR_PON_2; //set bit 2 to 1
	
	// Disable MUX_SEG of LCD_CR (0)
	LCD->CR &= ~(LCD_CR_MUX_SEG); //disable MUX_SEG
	
	// Set voltage source as internal for LCD_CR (0)
	LCD->CR &= ~(LCD_CR_VSEL); //clear bits (set to 0)
	
	// Wait until FCRSF flag of LCD_SR bit is set
	while(!(LCD->SR & LCD_SR_FCRSR));//while(LCD->SR && LCD_SR_FCRSF == 0);
	
	// Enable LCD by setting LCDEN of LCD_CR
	LCD->CR |= LCD_CR_LCDEN; //enable LCD
	
	// Wait until LCD is enabled by checking the ENS bit of LCD_SR
	while(!(LCD->SR & LCD_SR_ENS));//while(LCD->SR && LCD_SR_ENS == 0);
	
	// Wait until LCD booster is ready by checking the RDY bit of LCD_SR
	while(!(LCD->SR & LCD_SR_RDY));//while(LCD->SR && LCD_SR_RDY == 0);
}
...
void LCD_DisplayString(uint8_t* ptr){
	...
	for(i = 0; i < 6; i++){
		...
		//print current char
		LCD_WriteChar(ptr+i,dot,colon,i);
		
		if(colon) ptr++; //If the next char is a colon, the colon will already be printed, increment again
		if(dot) ptr++; //If the next char is a dot, the colon will already be printed, increment again
	}
}

void LCD_Display_Name(void){
	uint8_t name[6] = {'H', 'e', 'l', 'l','o','o'};
	LCD_DisplayString(name);
	
}

//convert time in milliseconds to time in minutes, seconds, milliseconds
void LCD_ConvTime(uint32_t time) {
	//convert time in msec to min sec msec
	uint32_t minutes = time / 60000;
	uint32_t seconds = (time % 60000) / 1000;	//5 digits originally, divide by 1000 to chop off 100, 10, and 1s place
	uint32_t milliseconds = ((time % 60000) % 1000) / 10; //3 digits originally, divide by 10 to chop off 1s place and prevent buffer overflow
	
	uint8_t string[8];
	//(char*) is tyoe casting string as a char pointer
	//springf sends formatted output to string pointed to by first argument
	sprintf((char*)string, "%02u:%02u.%02u", minutes, seconds, milliseconds);
	
	LCD_WriteChar(&string[7], 0, 0, 5);
	LCD_WriteChar(&string[6], 0, 0, 4);
	LCD_WriteChar(&string[4], 1, 0, 3);
	LCD_WriteChar(&string[3], 0, 0, 2);
	LCD_WriteChar(&string[1], 0, 1, 1);
	LCD_WriteChar(&string[0], 0, 0, 0);
